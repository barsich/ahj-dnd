...
export default class TaskManager {
  ...
  static onClick(event) {
    ...
    } else if (event.target.classList.contains('tasklist__task-cancel-button')) {
  ...
  static showAddTaskBlock(event) {
    ...
    event.target.closest('.tasklist__footer').appendChild(TaskManagerMarkups.addTaskBlock());
  }
  ...
  addTask(event) {
    ...
    const tasklist = event.target.closest('.tasklist__bar').querySelector('.tasklist__list');
    tasklist.appendChild(task);

    const hidden = event.target.closest('.tasklist__footer').querySelector('.tasklist__add-task');
    hidden.classList.remove('hidden');
    event.target.closest('.tasklist__add-task-block').remove();
    this.constructor.saveTaskList(this.todoTasks, this.inProgressTasks, this.doneTasks);
  }
  ...
  static saveTaskList(todo, inProgress, done) {
    ...
    [...inProgress].forEach((task) => inProgressList.push(task.textContent.slice(0, -1)));
